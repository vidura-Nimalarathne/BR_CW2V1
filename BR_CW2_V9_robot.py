...
class ARAP:
   ...
   #generalisation done.----------------------------------
    ...
    def handle_resource(self, resource_name, resource_value, last_random_time, left_speed, right_speed, resource_timer, 
        ...
        """
        Returns:
            resource_value, last_random_time, left_speed, right_speed, resource_timer,
            handled(bool),
            wants_seek(bool),
            seek_args(tuple|None),      # (resource_name, color, rotate_duration, color_threshold)
            wants_lock_on(bool),
            lock_on_args(tuple|None)    # (color, timeout)
        """
        if resource_value < threshold:
            ...
            if request_seek_and_lock_on:
                # Defer to the controller's new behaviors
